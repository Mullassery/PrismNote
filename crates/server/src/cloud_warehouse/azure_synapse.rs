...
async fn connect(conn: &CloudWarehouseConnection) -> Result<Client<tokio_util::compat::Compat<TcpStream>>> {
    let host = conn.host.as_deref().ok_or_else(|| anyhow!("Azure Synapse connection is missing host"))?;
    ...
    if conn.credentials.get("trust_server_certificate").map(String::as_str) == Some("true") {
        config.trust_cert();
    }

    let tcp = TcpStream::connect(config.get_addr()).await.context("TCP connection to Azure Synapse failed")?;
    tcp.set_nodelay(true).ok();
    Client::connect(config, tcp.compat_write()).await.context("Azure Synapse TDS handshake/login failed")
}

fn cell_to_json(row: &Row, idx: usize, col_type: ColumnType) -> Value {
    use ColumnType::*;
    match col_type {
        Bit | Bitn => row.get::<bool, _>(idx).map(Value::from).unwrap_or(Value::Null),
        Int1 => row.get::<u8, _>(idx).map(Value::from).unwrap_or(Value::Null),
        Int2 => row.get::<i16, _>(idx).map(Value::from).unwrap_or(Value::Null),
        Int4 => row.get::<i32, _>(idx).map(Value::from).unwrap_or(Value::Null),
        Int8 | Intn => row.get::<i64, _>(idx).map(Value::from).unwrap_or(Value::Null),
        Float4 => row.get::<f32, _>(idx).map(|v| Value::from(v as f64)).unwrap_or(Value::Null),
        Float8 | Floatn => row.get::<f64, _>(idx).map(Value::from).unwrap_or(Value::Null),
        NVarchar | NChar | BigVarChar | BigChar | Text | NText => {
            row.get::<&str, _>(idx).map(|v| Value::from(v.to_string())).unwrap_or(Value::Null)
        }
...
fn rows_to_result(rows: &[Row], started: std::time::Instant) -> CloudQueryResult {
    ...
    let col_types: Vec<ColumnType> =
        rows.first().map(|r| r.columns().iter().map(|c| c.column_type()).collect()).unwrap_or_default();

    let json_rows: Vec<Vec<Value>> = rows
        .iter()
        .map(|row| (0..row.columns().len()).map(|i| cell_to_json(row, i, col_types[i])).collect())
        .collect();
...
pub async fn test_connection(conn: &CloudWarehouseConnection) -> Result<String> {
    ...
    client.simple_query("SELECT 1").await.context("Azure Synapse test query failed")?;
...
pub async fn execute_query(conn: &CloudWarehouseConnection, query: &str) -> Result<CloudQueryResult> {
    ...
    let mut client = connect(conn).await?;
    let stream = client.simple_query(query).await.context("Azure Synapse query execution failed")?;
    let rows = stream.into_first_result().await.context("failed to collect Azure Synapse result rows")?;
...
mod tests {
    ...
    async fn live_connection_smoke_test() {
        let host = std::env::var("SYNAPSE_TEST_HOST").expect("set SYNAPSE_TEST_HOST to run this test");
        let mut conn = test_conn();
        conn.host = Some(host);
        conn.username = std::env::var("SYNAPSE_TEST_USER").unwrap();
        conn.password = std::env::var("SYNAPSE_TEST_PASSWORD").unwrap();

        let result = execute_query(&conn, "SELECT 1 AS one, 'hello' AS greeting").await.unwrap();
